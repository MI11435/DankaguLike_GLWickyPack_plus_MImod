...
execute.onloaded = function()
	...
	LaneEffect = GameObject("LaneEffect")
	LaneEffect.gameObject.transform:SetParent(util.GetLaneSpriteCanvas().transform, false)
	for i = 0, 6 do
		local lane = GameObject.Instantiate(LaneSpritePrefab)
		lane.gameObject.transform:SetParent(LaneEffect.transform, false)
		lane.name = "LaneEffect (" .. i .. ")"
...
function execute.start()
	LaneEffect.transform:SetSiblingIndex(0)
end
